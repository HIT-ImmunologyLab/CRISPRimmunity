...
def diamond_blastp(prog_dir,file,outfile,database,format,evalue):
	num_threads = 20
	script = prog_dir + "/software/diamond blastp -d "+database+" -q "+file+" -f "+str(format)+" -e "+str(evalue)+" -o "+outfile+" -p "+str(num_threads)+" --max-target-seqs 1"
	os.system(script)

# def blastp(prog_dir,file,outfile,database,format,evalue):
# 	num_threads = 20
# 	script = prog_dir + "/software/blastp -db "+database+" -query "+file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile+" -num_threads "+str(num_threads)+" -max_target_seqs 1"
#
# 	os.system(script)

# def diamond_blastx(prog_dir,file,outfile,database,format,evalue):
# 	num_threads = 20
# 	script = prog_dir + "/software/diamond blastx -d "+database+" -q "+file+" -f "+str(format)+" -e "+str(evalue)+" -o "+outfile+" -p "+str(num_threads)+" --max-target-seqs 1"
# 	os.system(script)

# def blastn(prog_dir,file,outfile,database,format,evalue):
# 	num_threads = 20
# 	format = 0
# 	script = prog_dir + "/software/blastn -db "+database+" -query "+file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile+" -num_threads "+str(num_threads)+" -num_alignments 1 -word_size 11"
# 	os.system(script)

# def blastn_file(prog_dir,file,outfile,subject_file,format,evalue):
# 	num_threads = 20
# 	format = 0
# 	script = prog_dir + "/software/blastn -subject "+subject_file+" -query "+file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile+" -word_size 11"
# 	print(script)
# 	os.system(script)

# def blastp_file(prog_dir,file,outfile,subject_file,format,evalue):
# 	num_threads = 20
# 	script = prog_dir + "/software/blastp -subject "+subject_file+" -query "+file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile+" -max_target_seqs 1"
# 	os.system(script)

# def diamond_mkdb(prog_dir,file,outfile):
# 	script = prog_dir + "/software/diamond --in "+file+" -d "+outfile
# 	os.system(script)

# def mkdb_blastn(prog_dir,file,outfile):
# 	script = prog_dir + "/software/makeblastdb -in "+file +" -dbtype nucl -out "+outfile
# 	os.system(script)

# def mkdb_blastp(prog_dir,file,outfile):
# 	script = prog_dir + "/software/makeblastdb -in "+file +" -dbtype prot -out "+outfile
# 	os.system(script)


# def get_def_phage_ncbi(def_file,acc_id):
# 	def_file = "/zrom1/WebServer/db/phage/dataFile/ncbi/refseq/phage_ncbi_refseq_def_info.txt"
# 	with open(def_file) as f:
# 		contents = f.read()
# 	def_info_s = contents.find(acc_id)
# 	if def_info_s!=-1:
# 		def_info_e = contents.find("\n",def_info_s)
# 		def_info = contents[def_info_s:def_info_e].split("\t")[-1].strip()
# 		return def_info
# 	else:
# 		return "NA"
...
def get_uniprot_def(prog_dir):
	file = "%s/data/UniProt-Virus-Phage/uniprot_species.txt"%(prog_dir)
	with open(file) as f:
		contents = f.readlines()
	proteins_species = {}
	for line in contents:
...
def get_protein_to_position_genbank(strain_id,pro_file,start,end,outfile,method):
...
def get_protein_to_position_fasta(strain_id,pro_file,start,end,outfile,method):
	f_result = open(outfile,'w')
	with open(pro_file) as f:
		contents = f.read().strip().split('>')
	for line in contents[1:]:
...
def short_blastn(prog_dir,file1,file2,outfile):
	command = prog_dir + "/software/blastn -query "+file1+" -subject "+file2+" -out "+outfile+" -task blastn-short -evalue 1000"
	os.system(command)

def identify_att(prog_dir,strain_id,bac_fna_file,bac_faa_file,prophage_region_detail_file,outdir,prefix):
	if os.path.exists(prophage_region_detail_file):
		...
		for region in contents[1:]:
			...
			if 'integrase' in key_protein:
				...
				short_blastn(prog_dir,up_file,down_file,out_blastn_file)
				out_att_file = os.path.join(region_dir,'att_info.txt')
				get_att_0(out_blastn_file,out_att_file)

def decide_boundary(prog_dir,strain_id,bac_fna_file,bac_faa_file,prophage_region_detail_file,att_dir,save_prophage_file,save_prophage_summary_file,save_prophage_protein_file,save_prophage_nucl_file,bac_protein_blastp_phage_db_file,bac_protein_def_file,prefix):
	bac_protein_homo_dict = {}
	if not os.path.exists(bac_protein_blastp_phage_db_file):
		print('0 prophage is detected!')
		sys.exit(1)
	...
	uniprot_proteins_species = get_uniprot_def(prog_dir)
	...
	f_save_summary.write('bacteria_id\tbacteria_def\tgenome_size\tprophage_start\tprophage_end\tmethod\tkey_proteins\tbest_hit_species\tCDS_number\tattl_region\tattr_region\n')
	...
	if os.path.exists(prophage_region_detail_file):
		...
		for region in new_prophage_regions:
			...
			region_end = region[1]
			if (len(new_merge_region)==0) or (region_start-new_merge_region[-1][1])>int(distance):
		...
		for region_index,region in enumerate(new_merge_region):
			...
			region_pro_sequence_list,prophage_pro_num = get_protein_to_position_genbank(strain_id,bac_faa_file,prophage_start,prophage_end,c_save_prophage_protein_file,'DBSCAN-SWA')
			...
			for index,region_pro in enumerate(region_pro_sequence_list[0::2]):
				region_pro = region_pro.split('|')[0:2]+[region_pro.split('|')[3],region_pro.split('|')[2]]+region_pro.split('|')[4:]
				f_save_prophage_protein.write('|'.join(region_pro)+'\n'+region_pro_sequence_list[2*index+1].strip()+'\n')
			...
			f_save.write('\t'.join(bac_info)+'\t'+'\t'.join([prophage_start,prophage_end,bac_info[2],key_words,hit_mf,str(prophage_pro_num),wide_attl,wide_attr])+'\n')
			f_save.flush()
			
			f_save_summary.write('\t'.join(bac_info)+'\t'+'\t'.join([prophage_start,prophage_end,'DBSCAN-SWA',key_words,hit_mf,str(prophage_pro_num),wide_attl,wide_attr])+'\n')	
...
def annotate_bacteria_fasta(prog_dir,fasta_file,outdir,prefix,kingdom):
	command = "%s/software/prokka %s --outdir %s --prefix %s --kingdom %s --force"%(prog_dir,fasta_file,outdir,prefix,kingdom)
	print(command)
	os.system(command)

def rpsblastp11proteins(fileName,outFileName):
    blast_cline = "/home/qiusuo/miniconda2/pkgs/blast-2.2.31-1/bin/rpsblast -query %s -comp_based_stats 0 -max_target_seqs 1 -evalue 0.01 -seg no -outfmt 5 -num_threads 20 -db /zrom/z-tools/data/cdd/Cdd -out %s" % (fileName,outFileName)
    os.system(blast_cline)

def get_bac_protein(prog_dir,strain_id,inputfile_bac,protein_dir,strain_type,save_prefix,add_genome_id='no',kingdom='Bacteria'):
	if strain_type == 'fasta':
		#protein_dir = os.path.join(outdir,'protein_annotation')
		annotate_bacteria_fasta(prog_dir,inputfile_bac,protein_dir,save_prefix,kingdom) #prokka
...
def get_phage_like_gene(prog_dir,bac_protein_file,phage_gene_annotation_dir,c_prefix):
	blastp_file = os.path.join(phage_gene_annotation_dir,prefix+'_blastp_uniprot.txt')
	correct_blastp_file = os.path.join(phage_gene_annotation_dir,c_prefix+'_blastp_uniprot.txt')
	if os.path.exists(blastp_file):
		os.rename(blastp_file,correct_blastp_file)
	else:
		blastp_file = correct_blastp_file
	database = "%s/database/DBSCAN-SWA_db/uniprot.dmnd"%(prog_dir)
	diamond_blastp(prog_dir,bac_protein_file,blastp_file,database,6,str(blastp_evalue))
	if os.path.exists(blastp_file):
...
def predict_prophage_dbscan_swa(prog_dir,strain_id,bac_fna_file,bac_faa_file,dbscan_region_file,swa_region_file,bac_protein_def_file,outdir,prefix):
...
''')
	...
	for inf in merge_inf:
		...
		region_pro_sequence_list,prophage_pro_num = get_protein_to_position_genbank(strain_id,bac_faa_file,inf['place'],inf['end'],region_faa_file,'DBSCAN-SWA')
...
# def bac_blastn_phagedb(prog_dir,prophage_file,outfile):
# 	phage_database = "%s/database/phage_nucl/phage_nucl_db"%(prog_dir)
# 	format = 6
# 	evalue = 0.01
# 	script = prog_dir + "/software/blastn -db "+phage_database+" -query "+prophage_file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile+" -num_threads 20 -word_size 11 -perc_identity 70 -reward 1 -penalty -2 -gapopen 0 -gapextend 0 -max_target_seqs 1000000"
# 	os.system(script)
...
def	annotate_prophage_region(prog_dir,strain_id,prophage_region_file,prophage_protein_file,prophage_nucl_file,annotate_outdir,add_annotation,prefix):
	if add_annotation == 'PGPD':
		...
		if os.path.exists(add_annotation):
			...
			if contents[0]=='>':
				...
				try:
					get_bac_protein(prog_dir,strain_id,add_annotation,annotate_outdir,'fasta','phage',phage_id,'Viruses')
				except:
				...
				try:
					get_bac_protein(prog_dir,strain_id,add_annotation,annotate_outdir,'GenBank','phage',phage_id,'Viruses')
				except:
					print('parse %s error!'%add_annotation)
					sys.exit(1)
...
def predict_prophage(prog_dir,strain_id,inputfile_bac,outdir,add_annotation,prefix):
	start = time.time()
	...
	protein_dir = os.path.join(outdir,'protein_annotation')
	mkdir(protein_dir)
	get_bac_protein(prog_dir,strain_id,inputfile_bac,protein_dir,type,prefix)
	...
	phage_like_protein_list = get_phage_like_gene(prog_dir,bac_protein_file,phage_gene_annotation_dir,prefix)
	...
	predict_prophage_dbscan_swa(prog_dir,strain_id,bac_fna_file,bac_protein_file,dbscan_region_file,swa_region_file,bac_protein_annotation_file,prophage_outdir,prefix)
	...
	if os.path.exists(prophage_region_file):
		with open(prophage_region_file) as f:
	...
	identify_att(prog_dir,strain_id,bac_fna_file,bac_protein_file,prophage_region_detail_file,att_outdir,prefix)
	decide_boundary(prog_dir,strain_id,bac_fna_file,bac_protein_file,prophage_region_detail_file,att_outdir,save_prophage_file,save_prophage_summary_file,save_prophage_protein_file,save_prophage_nucl_file,bac_protein_blastp_phage_db_file,bac_protein_annotation_file,prefix)
	...
	if add_annotation!='none':
		...
		annotate_prophage_region(prog_dir,strain_id,save_prophage_file,save_prophage_protein_file,save_prophage_nucl_file,prophage_region_annotate_dir,add_annotation,prefix)
	else:
...
class MyThread(threading.Thread):
	def __init__(self,prog_dir,strain_id,inputfile_bac,c_outdir,add_annotation,c_prefix):
	...
	def run(self):
		print(self.c_outdir)
		predict_prophage(self.prog_dir,self.strain_id,self.inputfile_bac,self.c_outdir,self.add_annotation,self.c_prefix)

def trans_to_PHIS_overall(outdir):
	prophage_summary_file = os.path.join(outdir,prefix+'_DBSCAN-SWA_prophage_summary.txt')
	prophage_detail_file = os.path.join(outdir,prefix+'_DBSCAN-SWA_prophage.txt')
	
	prophage_fna_file = os.path.join(outdir,prefix+'_DBSCAN-SWA_prophage.fna')
	prophage_faa_file = os.path.join(outdir,prefix+'_DBSCAN-SWA_prophage.faa')
	
	prophage_sequence_dir = os.path.join(outdir,'prophage_sequences')
	mkdir(prophage_sequence_dir)
	with open(prophage_fna_file) as f:
		contents = f.read().strip().split('>')	
	for prophage in contents[1:]:
		prophage_line = prophage.split('\n')[0]
		prophage_location = prophage_line.split('|')[1]
		prophage_location = '_'.join(prophage_location.split(':'))
		c_prophage_fna_dir = os.path.join(prophage_sequence_dir,prophage_location)
		mkdir(c_prophage_fna_dir)
		c_prophage_fna_file = os.path.join(c_prophage_fna_dir,'prophage_region.fna')
		with open(c_prophage_fna_file,'w') as f:
			f.write('>'+prophage)

	with open(prophage_faa_file) as f:
		contents = f.read().strip().split('>')	
	for prophage in contents[1:]:
		prophage_line = prophage.split('\n')[0]
		prophage_location = prophage_line.split('|')[1]
		...
		c_prophage_fna_dir = os.path.join(prophage_sequence_dir,prophage_location)
		#mkdir(c_prophage_fna_dir)
		c_prophage_faa_file = os.path.join(c_prophage_fna_dir,'prophage_region.faa')
		with open(c_prophage_faa_file,'w') as f:
			f.write('>'+prophage)
...
if __name__=='__main__':
	...
	parser.add_argument('--distance', help='optional,the maximal distance of two neighbor clusters to merge in to a cluster.\n')
	parser.add_argument('--threads', help='optional,if a muti-fasta file is received, the <threads> will specify the number of contigs running simultaneously; default:1\n')
	...
	if args.threads:
		...
		if int(thread_num)>30:
			print('Warning:The thread number can not be larger than 30!the thread num has been set as 30')
			thread_num = 30
	else:
		thread_num = 1
	...
	if len(list(strain_inf_dict.keys()))==1:
		...
		predict_prophage(prog_dir,strain_id,inputfile_bac,outdir,add_annotation,prefix)
	else:
		counter = 1
		tsk = []
		...
		for strain_id in c_strain_dict.keys():
			...
			#try:
				tsk.append(MyThread(prog_dir,strain_id,c_inputfile_bac,c_outdir,add_annotation,c_prefix))
			# except:
		...
		f_save.write('prefix\tcontig_id\tcontig_def\tgenome_size\tprophage_start\tprophage_end\tmethod\tkey_proteins\tbest_hit_species\tCDS_number\tattl_region\tattr_region\n')
		f_save.flush()
